...
class Detector:
    ...
    def __init__(self, num_classes=91,train_weights="fasterrcnn_resnet50_fpn_coco.pth"):
        ...
        self.model.load_state_dict(torch.load(train_weights, map_location=self.device))
    ...
    def infer(self,image):
        ...
        # c  h  w  的tensor
        # img_tensor = torch.from_numpy(image / 255.).permute(2, 0, 1).float().cpu()
        img_tensor = torch.from_numpy(image / 255.).permute(2, 0, 1).float().to(self.device)
        # 增加一个维度
