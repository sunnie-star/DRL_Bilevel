...
class Server:
    ...
    # easy_perfrom_SR_detect_edsr 把config删除，没啥用
    def __init__(self,config=None,nframes=None):
        self.config = config
        ...
        self.SRconfig = self.load_configuration(
            '/home/ubuntu/VideoAnalytics/workplace/DRL_SR_Infer/backend/SRconfiguration.yml')
    ...
    def perform_detection_DRL(self, images_direc, resolution,s_frame, fnames=None, images=None):
        ...
        for fname in fnames:
            ...
            fid_ = fid + 1
            while fid_ < (s_frame[0] + self.config.batch_size):
                if fid_ not in s_frame:
                    for label, conf, (x, y, w, h) in detection_results:
                        r = Region(fid_, x, y, w, h, conf, label,
                                   resolution, origin="mpeg")
                        final_results.append(r)
                else:
                    break
    ...
    def perform_detection(self, images_direc, resolution,s_frame, fnames=None, images=None):
        #  results=self.perform_detection("server_temp",self.config.low_resolution,fnames)
        #  gt 模式需要用到
        final_results = Results()
        if fnames is None:
    ...
    # 提供精简版图片sr后再infer，为了测试drl  sr  infer时间

    def perform_edsr_on_video(self, video_name, fnames=None, images=None):
        #  results=self.perform_detection("server_temp",self.config.low_resolution,fnames)
        # SR 初始化
        self.SRconfig = self.load_configuration('backend/SRconfiguration.yml')
        self.SRconfig.scale = [2]
        print('self.SRconfig', self.SRconfig)

        final_results = Results()
        self.logger.info(f"Running inference on video with scale {self.SRconfig.scale}")
        t1 = time.time()
        #  SR model prepare==========================================
        self.logger.info('SR model prepare===========================================================')
        torch.manual_seed(self.SRconfig.seed)
        ...
        times=[]
        frame_id=0
        with VideoProcessor(video_name,0) as video:
            for frame in video:
                starttime=time.time()
                image, = common.set_channel(frame, n_channels=self.SRconfig.n_colors)
                image, = common.np2Tensor(image, rgb_range=self.SRconfig.rgb_range)
                image, = t.prepare(image.unsqueeze(0))
                sr = t.model(image, idx_scale=0)
                sr = quantize(sr, self.SRconfig.rgb_range).squeeze(0)
                ...
                print(f"finish sr {frame_id}")
                frame_id+=1
        return times
    def easy_perform_detection_edsr_on_video(self, video_name, fnames=None, images=None):
        #  results=self.perform_detection("server_temp",self.config.low_resolution,fnames)
        # SR 初始化
        self.SRconfig = self.load_configuration('backend/SRconfiguration.yml')
        self.SRconfig.scale = [2]
        ...
        with VideoProcessor(video_name,0) as video:
            for frame in video:
                ...
                ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
                endtime=time.time()
                times_sr.append(endtime-starttime)
                ...
                times_infer.append(endtime - starttime)
                results.append(detection_results)
                print(f"finish sr and infer{frame_id}")
                frame_id += 1

        return results,times_sr,times_infer

    def easy_perform_detection_edsr(self, images_direc, fnames,timer):
        #  results=self.perform_detection("server_temp",self.config.low_resolution,fnames)
        # SR 初始化
        # 注意这里的scale是数组
        self.SRconfig.scale = [2]
        self.SRconfig.data_test = ['Demo']
        self.SRconfig.pre_train = "../models/edsr_baseline_x2-1bc95232.pt"
        self.SRconfig.test_only = True
        self.SRconfig.save_results = False
        print('self.SRconfig', self.SRconfig)
        #  SR model prepare==========================================
        ...
        ckp = checkpoint(self.SRconfig)
        global SRmodel
        SRmodel = Model(self.SRconfig, ckp,timer)
        self.t = SRDetector(self.SRconfig, SRmodel, ckp)
        torch.set_grad_enabled(False)
        self.t.model.eval()
        if fnames is None:
            fnames = sorted(os.listdir(images_direc))
        ...
        for fname in fnames:
            ...
            if src_image is None:
                raise ValueError("can not read images!")
            # SR ======================================================
            timer.tic()
            t3 = time.time()
            image, = common.set_channel(src_image, n_channels=self.SRconfig.n_colors)
            image, = common.np2Tensor(image, rgb_range=self.SRconfig.rgb_range)
            image, = self.t.prepare(image.unsqueeze(0))
            sr = self.t.model(image, idx_scale=0)
            sr = quantize(sr, self.SRconfig.rgb_range).squeeze(0)
            normalized = sr * 255 / self.SRconfig.rgb_range
            ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
            timer.toc("sr")
            ...
            self.logger.info(f"{fid} image size:{src_image.shape[1]},{src_image.shape[0]};"
                             f"{ndarr.shape[1]},{ndarr.shape[0]}")
        ...
        print(f"SR numbers:{len(fnames)}\naverage edsr per frame time:{sr_time / len(fnames)}\n"
              f"average infer per frame time{infer_time/len(fnames)}")
        # f = open(f"{self.config.video_oname}_edsr_record.txt", "a+")
        # f.write(f"{sr_time / self.config.batch_size}    {infer_time / self.config.batch_size}\n")
        # f.close()
        return results
    ...
    def perform_detection_asr(self, images_direc, resolution, fnames=None, images=None):
        ...
        net = module.Net(multi_scale=self.ASRconfig.multi_scale,
        ...
        return final_results, time_of_asr_one_batch, time_of_infer_one_batch
    ...
    def perform_low_query(self, vid_data):
        ...
        if self.config.method == "edsr":
            ...
            self.logger.info(f"{self.config.method}++++++++++++++++++++++++++++++++++++++++++++++++")
            results, time_of_sr_one_batch, time_of_infer_one_batch = self.perform_detection_asr("server_temp", self.config.low_resolution, fnames)
        ...
        #     self.train_flag = False
        self.train_flag = False
        return {"results": detections_list, "whether_train": self.train_flag, "time_of_decode_one_batch": time_of_decode_one_batch, "time_of_sr_one_batch": time_of_sr_one_batch, "time_of_infer_one_batch": time_of_infer_one_batch}
