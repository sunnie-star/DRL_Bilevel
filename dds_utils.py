    ...
    for _, regions in results_dict.items():
        ...
        for r in regions:
            if r.conf < min_conf_threshold:
                to_remove.append(r)
        for r in to_remove:
            regions.remove(r)   #  remove result.regions_dict and result.regions
...
def merge_boxes(results_dict, min_conf_threshold, iou_threshold):
    final_results = Results()

    # Clean dict to remove min_conf_threshold
    for regions in results_dict:
        to_remove = []
        for r in regions:
            if r[1] < min_conf_threshold:
                to_remove.append(r)
        for r in to_remove:
...
def filter_results(bboxes, gt_flag, gt_confid_thresh, mpeg_confid_thresh,
    ...
    relevant_classes = ["vehicle",'roadside-objects']
    if gt_flag:
...
def evaluate(max_fid, map_dd, map_gt,
    ...
    for fid in range(max_fid+1):
        ...
        #f1_list.append(round((2.0*tp/(2.0*tp+fp+fn)), 3))
    tp = sum(tp_list)
    fp = sum(fp_list)
    fn = sum(fn_list)
