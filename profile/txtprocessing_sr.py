...
import copy
f=open("srparam.txt","r",encoding="utf-8")
y_tensor_shape=[]
y_params=[]
y_name=[]
for line in f:
    line.strip()
    #不等于-1说明找到了
    if line.find("==")!=-1 or line.find("Sequential")!=-1 or line.find("EDSR")!=-1 or line.find("Model")!=-1:
        continue
...
plt.tight_layout()
plt.savefig('txtprocessing_sr.png',format="png", bbox_inches='tight') # 替换 plt.show()
# plt.savefig('Bindwidth.pdf',format="pdf", bbox_inches='tight') # 替换 plt.show()
plt.savefig("txtprocessing_sr.svg", format="svg")
plt.show()