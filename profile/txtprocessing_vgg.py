# torchinfo打印的模型参数是recursive的不算 参数量
import copy
f=open("vggparam.txt","r",encoding="utf-8")
y_tensor_shape=[]
y_params=[]
y_name=[]
for line in f:
    line.strip()
    #不等于-1说明找到了
    if line.find("==")!=-1 or line.find("1-")!=-1 or line.find("VGG")!=-1:
        continue
...
plt.tight_layout()
plt.savefig('txtprocessing.png',format="png", bbox_inches='tight') # 替换 plt.show()
# plt.savefig('Bindwidth.pdf',format="pdf", bbox_inches='tight') # 替换 plt.show()
plt.savefig("txtprocessing.svg", format="svg")
plt.show()